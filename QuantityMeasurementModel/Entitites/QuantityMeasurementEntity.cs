...
namespace QuantityMeasurementModel
{
    ...
    //stores what all calculations that happened
    {
        public Guid Id { get; } //guid--globally unique identifier very big num
        ...
        // ── UC16: private full constructor used by database hence prvt
        // without generating a new Id / Timestamp 

        private QuantityMeasurementEntity(Guid id, DateTime timestamp, string operation,
        ...
        public static QuantityMeasurementEntity Reconstruct(
            Guid id, DateTime timestamp, string operation, QuantityDTO? operand1, QuantityDTO? operand2, QuantityDTO? result,
            bool? boolResult, double? scalarResult, bool hasError, string errorMessage)
        // Rebuild object from saved data
        {
        ...
        {
            if (HasError) return $"[{Timestamp:u}] {Operation} ERROR: {ErrorMessage}";//error statement
            ...
            //Timestamp:u--u=utc universal time format
        }
    }
}
